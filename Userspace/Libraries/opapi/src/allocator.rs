...
unsafe impl GlobalAlloc for LockedAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if result == ptr::null_mut() {
            let extend_size = if layout.size().next_power_of_two() <= 16384 {16384} else {layout.size().next_power_of_two()};
            let heapspace = crate::syscall::mmap(0,extend_size,3,1 | 8,-1,0);
            if heapspace.is_positive() {
                lock.init(heapspace as usize,extend_size);
...
#[global_allocator]
static GLOBAL: LockedAllocator = LockedAllocator(Mutex::new(LinkedListAllocator::new()));
